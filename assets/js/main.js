...
/* borrador jime
const nombres = people.map(({name}) => name)
console.log(nombres);
*/
/* borrador jime
const sonSenior = people.filter(({role}) => role === 'senior');
console.log(sonSenior.length)
*/
/* borrador jime
const palabraSenior = people.filter(({role}) => role === 'senior').map(({role}) => role);
console.log('para que me diga la palabra senior -->>>', palabraSenior[0]);
*/

const message = `Hay ${people.length} personas en el ${teamName} team. \n Sus nombres son ${people.map(({name}) => name)} .\n ${people.filter(({role}) => role === 'senior').length} son senior con experiencia.`
...
const detectCollision = (objects, {x: pointX, y: pointY}) => {
 ...
 // --> comento para hacer el destructuring en el parametro de la funcion:  const { x: pointX, y: pointY } = point
   return  objects.find(({x,y,width,height} ) => pointX >= x && pointX <= x + width &&
   pointY >= y && pointY <= y + height) || false
...
const {fname: nombreDelPerfil, lname: apellidoDelPerfil , social: linksDelPerfil} = profile;

console.log('Ejercico _3_a  ->', `El link de twitter de ${nombreDelPerfil} ${apellidoDelPerfil} es: ${linksDelPerfil.twitter}` ); // https://twitter.com/hearsparkbox
...
console.log('Ejercicio _3_c ->', `El resultado de las operaciones es`, multiplicacion);
...
// borrador jime: console.log('Ejercicio _4_a ->','los que pasan a la semi final son: ', semiFinal);
const final = semiFinal.filter(({pais}) => pais == 'Argentina' || pais == 'Brasil' ? pais : false);
// borrador jime: console.log('Ejercicio _4_b ->','argentina y brasil son', final);
...
/* borrador jime
asi traigo para saber como son los goles --> console.log(finalResultado[0][1]); --> porque con finalResultado[0]. goles no funciona
const campeon = (finalResultado) => finalResultado[0].goles > finalResultado[1].goles ? `El campeón es ${finalResultado[0].pais}` : `El campeón es ${finalResultado[1].pais}` ;
*/
const campeon = (finalResultado /* aca podria tener otro nombre la variable */) => { return finalResultado[0][1] > finalResultado[1][1] ? `El campeón es ${finalResultado[0][0]}` : `El campeón es ${finalResultado[1][0]}` };
